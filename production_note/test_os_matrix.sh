...
assert_contains "$output" "kernel.pid_max = 64000"
assert_contains "$output" "net.ipv4.tcp_keepalive_time = 120"
assert_contains "$output" "net.ipv4.tcp_fastopen = 3"
assert_contains "$output" "net.ipv4.tcp_retries2 = 5"
assert_contains "$output" "fs.file-max = 98000"
assert_contains "$output" "vm.max_map_count = 131060"
assert_contains "$output" "vm.zone_reclaim_mode = 0"
...
assert_contains "$output" "mongodb soft fsize unlimited"
assert_contains "$output" "mongodb hard cpu unlimited"
assert_contains "$output" "mongodb soft as unlimited"
assert_contains "$output" "mongodb hard memlock unlimited"
assert_contains "$output" "mongodb soft nofile 64000"
pass "MongoDB 7 dry-run disables THP, uses NCP sysctl baseline, and Ubuntu mongodb limits user"
